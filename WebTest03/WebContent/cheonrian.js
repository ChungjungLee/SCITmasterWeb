...
function validation() {
	...
	/* 휴대폰 번호 확인 */
	var eArrTelphone = document.getElementsByName('telphone');
	if (eArrTelphone[0].selectedIndex == 0) {
		alert('휴대폰 앞번호를 선택해 주세요');
		return false;
	}
	for (var i = 1; i < eArrTelphone.length; i++) {
		if (eArrTelphone[i].value == "") {
	...
	info += "휴대폰 번호 확인: " + eArrTelphone[0].value + "-" + eArrTelphone[1].value + "-" + eArrTelphone[2].value + "\n";
	alert(info);
